...
def processTrips(pid,records):
    ...
    index1,neighbor=createindex('data.geojson')
    ...
    for row in reader:
        # pickup point
        if len(row)>=6 and row[5]!='NULL' and row[5]!='':
        
            p1= geom.Point(proj(float(row[3]),float(row[2]))) # pickup point 
        
            p2 = geom.Point(proj(float(row[5]),float(row[4]))) #dropoff point
...
if __name__=='__main__':
    ...
    .reduceByKey(lambda x,y:x+y)\
    .map(lambda x: (x[0],x[1][:6])).sortByKey()\
    .map(lambda x:(x[0],x[1][0],x[1][1],x[1][2],x[1][3],x[1][4],x[1][5])).saveAsTextFile(sys.argv[2])
